...
const MDEditorApp = ({ data, onUpdate, instanceId, title }) => {
  const [markdown, setMarkdown] = useState((data && data.content) || DEFAULT_MD);
  ...
  // ---------------------------------------------------------------------
  // Sync markdown -> parent (skip the very first mount so we never clobber
  // stored data with the initial state before file/content injection runs)
  // ---------------------------------------------------------------------
  useEffect(() => {
    if (!hasMountedRef.current) { hasMountedRef.current = true; return; }
    if (onUpdate) onUpdate({ content: markdown });
  }, [markdown]);

  // One-time restore of previously saved content on mount
  useEffect(() => {
    if (data && data.content && !data.fileData) {
      setMarkdown(data.content);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Reactive: a new file was dropped/opened into this window
  useEffect(() => {
  ...
  useEffect(() => {
    if (viewMode === 'edit') setActiveCell(null);
  }, [viewMode]);

  // Container query for responsive tab-only toolbar
  useEffect(() => {
    if (!containerRef.current) return;
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) setIsCompact(entry.contentRect.width < 640);
    });
  ...
  useEffect(() => {
    ...
    const loadDependencies = async () => {
      addCss('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Roboto:wght@400;500;700&family=Merriweather:wght@400;700&family=Open+Sans:wght@400;600&family=Fira+Code:wght@400;500&family=JetBrains+Mono:wght@400;500&display=swap');
      ...
      try {
        ...
        // Guard against double-registration when multiple editor windows are open
        if (window.marked && window.markedKatex && !window.__mdEditorKatexRegistered) {
          const ext = typeof window.markedKatex === 'function' ? window.markedKatex : window.markedKatex.default;
          if (ext) {
            window.marked.use(ext({ throwOnError: false }));
            window.__mdEditorKatexRegistered = true;
          }
        }

        if (!cancelled) setIsReady(true);
      } catch (err) {
        console.error('MdEditor: failed to load core scripts', err);
        if (!cancelled) {
          showNotification('Some editor components failed to load.', 'error');
          setIsReady(true);
        }
      }
    };
  ...
  const modifyTable = (action) => {
    ...
    if (!document.body.contains(tableNode)) { setActiveCell(null); return; }
    ...
    if (action === 'deleteRow') {
      if (rowIndex === -1) { showNotification('Cannot delete header row.', 'error'); return; }
      ...
      if (allTrs[0].children.length <= 1) { showNotification('Cannot delete the last column.', 'error'); return; }
      allTrs.forEach((tr) => { if (tr.children[colIndex]) tr.children[colIndex].remove(); });
      modified = true;
      setActiveCell(null);
    ...
    if (modified) syncTableToMarkdown(tableNode, tableIndex);
  };

  // ---------------------------------------------------------------------
  // Parse markdown -> sanitized HTML (Step 1)
  // ---------------------------------------------------------------------
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    preBlocks.forEach((pre) => {
      ...
      dotsWrap.className = 'flex items-center gap-1.5 mr-3';
      ['bg-red-400 dark:bg-[#ff5f56] border-red-200 dark:border-[#e0443e]',
       'bg-yellow-400 dark:bg-[#ffbd2e] border-yellow-200 dark:border-[#dea123]',
       'bg-green-400 dark:bg-[#27c93f] border-green-200 dark:border-[#1aab29]'].forEach((cls) => {
        const dot = document.createElement('div');
        dot.className = 'w-3 h-3 rounded-full border ' + cls;
        dotsWrap.appendChild(dot);
      });
      const chevron = document.createElement('i');
      chevron.className = 'fa-solid fa-chevron-down toggle-icon text-[10px] text-slate-400 dark:text-zinc-500 group-hover:text-slate-600 dark:group-hover:text-zinc-300 transition-colors';
      const langLabel = document.createElement('span');
      langLabel.className = 'text-xs font-semibold tracking-wider text-slate-600 dark:text-zinc-300 font-mono ml-1';
      langLabel.innerText = lang;
      leftGroup.appendChild(dotsWrap);
      ...
      copyBtn.onclick = (e) => {
        ...
        try {
          ...
          setTimeout(() => { copyBtn.innerHTML = '<i class="fa-regular fa-copy"></i> <span>Copy</span>'; }, 2000);
        } catch (err) {
    ...
    tables.forEach((table, tableIndex) => {
      ...
      const attachEvents = (cell, isHeader, rIdx, cIdx) => {
        ...
        cell.addEventListener('focus', () => {
          setActiveCell({ tableIndex, tableNode: table, rowIndex: isHeader ? -1 : rIdx, colIndex: cIdx });
        });

        cell.addEventListener('focusout', (e) => {
          const wrapperNode = table.closest('.interactive-table-wrapper');
          if (e.relatedTarget && ((wrapperNode && wrapperNode.contains(e.relatedTarget)) || e.relatedTarget.closest('.table-toolbar'))) {
            return;
          }
          setTimeout(() => { if (table.isConnected) syncTableToMarkdown(table, tableIndex); }, 100);
        });
      };

      const theadTr = table.querySelector('thead tr');
      if (theadTr) Array.from(theadTr.children).forEach((th, cIdx) => attachEvents(th, true, -1, cIdx));
  ...
  // Click outside any table/toolbar clears active cell toolbar
  useEffect(() => {
  ...
  // ---------------------------------------------------------------------
  // Intelligent text editing (auto-pair brackets/quotes, tab, backspace)
  // ---------------------------------------------------------------------
  const PAIRS = { '(': ')', '[': ']', '{': '}', '"': '"', "'": "'", '*': '*', '_': '_', '~': '~', '<': '>' };
  const STEP_OVER = [')', ']', '}', '>', '"', "'", '*', '_', '~'];
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'Tab') {
      e.preventDefault();
      const newVal = markdown.substring(0, start) + '  ' + markdown.substring(end);
      setMarkdown(newVal);
      setTimeout(() => { textarea.selectionStart = textarea.selectionEnd = start + 2; textarea.scrollTop = scrollTop; }, 0);
      return;
    }
    ...
    if (e.key === 'Backspace' && start === end && start > 0) {
      ...
      if (PAIRS[prevChar] && PAIRS[prevChar] === nextChar) {
        ...
        setMarkdown(newMd);
        setTimeout(() => { textarea.selectionStart = textarea.selectionEnd = start - 1; textarea.scrollTop = scrollTop; }, 0);
        return;
      }
    }
  ...
  const handleChange = (e) => {
    ...
    const data_ = e.nativeEvent && e.nativeEvent.data;

    if (inputType === 'insertText' && data_ != null && data_.length === 1) {
      if (STEP_OVER.includes(data_) && val[start] === data_) {
  ...
  const executeReplaceAll = () => {
    ...
    try {
      if (matchMode === 'exact') {
        ...
        setMarkdown(parts.join(replaceText));
        ...
        newMd = markdown.replace(regex, (...args) => {
          ...
          result = result.replace(/\\{(ANY|WORD|NUMBER)\\}/g, (m) => {
            ...
            for (let i = 0; i < wildcards.length; i++) {
              if (wildcards[i] === m) {
                if (occurrence === usageCount[m]) { usageCount[m]++; return args[i + 1] !== undefined ? args[i + 1] : m; }
                occurrence++;
              }
            }
            return m;
  ...
  // Drag-resize divider
  // ---------------------------------------------------------------------
  const startDrag = (e) => { isDraggingRef.current = true; e.preventDefault(); };
  ...
  const exportPDF = async () => {
    ...
    clone.querySelectorAll('.code-content').forEach((el) => el.classList.remove('hidden'));
    clone.querySelectorAll('.toggle-icon').forEach((el) => { el.classList.remove('fa-chevron-right'); el.classList.add('fa-chevron-down'); });
  ...
  const exportDOCX = async () => {
    if (!window.docx) { showNotification('DOCX library is still loading.', 'error'); return; }
    ...
    try {
      ...
      const buildTextRuns = (inlineTokens, formatOpts) => {
        ...
        inlineTokens.forEach((t) => {
          const currentOpts = Object.assign({}, formatOpts);
          ...
          if (t.type === 'link') { currentOpts.color = '0563C1'; currentOpts.underline = { type: 'single' }; }

          if (t.tokens && t.tokens.length > 0) {
            runs = runs.concat(buildTextRuns(t.tokens, currentOpts));
          } else {
            const rawText = t.text || t.raw || '';
            runs.push(new TextRun(Object.assign({ text: rawText }, currentOpts)));
          }
        });
        return runs;
      };

      const processBlockTokens = (tokensArray, listLevel) => {
            ...
            case 'list':
              token.items.forEach((item) => {
                ...
                item.tokens.forEach((itemToken) => {
                  if (itemToken.type === 'text') itemRuns = itemRuns.concat(buildTextRuns(itemToken.tokens || [{ type: 'text', raw: itemToken.text }]));
                  else nestedBlocks = nestedBlocks.concat(processBlockTokens([itemToken], listLevel + 1));
                });
            ...
            case 'table': {
              ...
              tableRows.push(new TableRow({
                children: token.header.map((cell) => new TableCell({
                  children: [new Paragraph({ children: buildTextRuns(cell.tokens), bold: true })],
                  shading: { fill: 'F3F4F6' }, margins: { top: 100, bottom: 100, left: 100, right: 100 }
                }))
              }));
              token.rows.forEach((row) => {
                tableRows.push(new TableRow({
                  children: row.map((cell) => new TableCell({
                    children: [new Paragraph({ children: buildTextRuns(cell.tokens) })],
      ...
      const doc = new Document({
        numbering: {
          config: [
            { reference: 'unordered-list', levels: Array.from({ length: 6 }).map((_, i) => ({ level: i, format: 'bullet', text: i % 2 === 0 ? String.fromCharCode(8226) : String.fromCharCode(9702), alignment: 'start', style: { paragraph: { indent: { left: 720 * (i + 1), hanging: 360 } } } })) },
  ...
  const styleText = [
    ...
    ".pdf-export-clone blockquote { border-left-color:#dfe2e5 !important; background:#f9fafb !important; color:#6a737d !important; }",
    ".pdf-export-clone code { background-color: rgba(27,31,35,0.08) !important; color:#1e293b !important; }",
    ".pdf-export-clone .code-block-wrapper { background:#f8fafc !important; border-color:#e2e8f0 !important; page-break-inside: avoid; break-inside: avoid; }",
    ".pdf-export-clone .code-block-wrapper > div:first-child { background:#f1f5f9 !important; border-bottom-color:#e2e8f0 !important; }",
    ".pdf-export-clone .code-block-wrapper > div:first-child span { color:#334155 !important; }",
    ".pdf-export-clone .code-copy-btn, .pdf-export-clone .toggle-icon { display:none !important; }",
    ".pdf-export-clone table { border-color:#cbd5e1 !important; page-break-inside: auto; }",
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-50 dark:bg-zinc-950 overflow-hidden font-sans relative">
      ...
      <header className="bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 shadow-sm px-3 py-1.5 flex flex-row items-center justify-between gap-3 flex-shrink-0 overflow-x-auto whitespace-nowrap transition-colors">
        <div className="flex bg-slate-100 dark:bg-zinc-950 p-0.5 rounded-md border border-slate-200 dark:border-zinc-800 flex-shrink-0 transition-colors">
          <button onClick={() => { setViewMode('edit'); setSplitRatio(50); }}
            className={'flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium transition-colors ' + (viewMode === 'edit' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100')}>
            <i className="fa-solid fa-pen"></i> {!isCompact && <span>Edit</span>}
          </button>
          <button onClick={() => { setViewMode('split'); setSplitRatio(50); }}
            className={'flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium transition-colors ' + (viewMode === 'split' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100')}>
            <i className="fa-solid fa-columns"></i> {!isCompact && <span>Split</span>}
          </button>
          <button onClick={() => { setViewMode('preview'); setSplitRatio(50); }}
            className={'flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-medium transition-colors ' + (viewMode === 'preview' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-zinc-100')}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button onClick={() => setShowFindReplace(!showFindReplace)}
            className={'flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium rounded transition-colors ' + (showFindReplace ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-800/50' : 'text-slate-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800')}>
          ...
          <input type="file" accept=".md,.txt" ref={fileInputRef} onChange={handleImportFile} className="hidden" />
          <button onClick={() => fileInputRef.current.click()}
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-slate-700 dark:text-zinc-300 bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-700 rounded hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors">
          ...
          <button onClick={exportDOCX}
            className="flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 border border-blue-600 rounded transition-colors shadow-sm">
      ...
      {notification && (
        <div className="absolute top-14 left-1/2 transform -translate-x-1/2 z-50 animate-bounce">
      ...
      <main ref={containerRef} className={'flex-1 flex ' + (isCompact ? 'flex-col' : 'flex-row') + ' overflow-hidden min-h-0 min-w-0 relative'}>
        <div style={{ flexBasis: viewMode === 'split' ? (splitRatio + '%') : '100%', display: viewMode === 'preview' ? 'none' : 'flex' }}
          ...
          <div className="flex items-center justify-between px-3 py-1 bg-[#252526] border-b border-[#333] flex-shrink-0">
            <span className="text-gray-300 font-medium text-xs flex items-center gap-1.5"><i className="fa-solid fa-code"></i> Editor</span>
            {viewMode === 'split' && (
          ...
          {showFindReplace && (
            <div className="absolute top-8 right-4 w-80 bg-white dark:bg-zinc-900 shadow-2xl rounded-xl border border-slate-200 dark:border-zinc-700 z-40 p-3 animate-slide-up flex flex-col gap-3 transition-colors">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-700 dark:text-zinc-200">Find and Replace</span>
              ...
              <div className="flex bg-slate-100 dark:bg-zinc-950 p-1 rounded-md text-xs border border-slate-200 dark:border-zinc-800">
                {['exact', 'smart', 'regex'].map((mode) => (
                  <button key={mode} onClick={() => setMatchMode(mode)}
                    className={'flex-1 py-1 rounded capitalize ' + (matchMode === mode ? 'bg-white dark:bg-zinc-800 shadow-sm font-bold text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-zinc-400 hover:text-slate-900')}>
                    {mode}
                  </button>
                ))}
              </div>
              ...
              <div className="text-[10px] text-slate-500 dark:text-zinc-400 bg-slate-50 dark:bg-zinc-950 p-2 rounded border border-slate-200 dark:border-zinc-800 leading-relaxed transition-colors">
                ...
                {matchMode === 'smart' && 'Smart Match: use {NUMBER}, {WORD}, or {ANY} as wildcards in Find, and {ANY} / {1} in Replace to reinsert matches.'}
                {matchMode === 'regex' && 'Regex: use standard regular expression syntax. Use $1, $2 in Replace for capture groups.'}
              </div>
              <div className="flex items-center justify-end mt-1">
                <button onClick={executeReplaceAll} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded shadow-sm transition-colors">Replace All</button>
              </div>
            </div>
          )}
        ...
        {viewMode === 'split' && (
          <div
            className={'flex-none bg-slate-200 dark:bg-zinc-800 hover:bg-blue-400 dark:hover:bg-blue-600 transition-colors z-20 flex items-center justify-center ' + (isCompact ? 'w-full h-3 cursor-row-resize' : 'w-2 h-full cursor-col-resize')}
        ...
        <div style={{ flexBasis: viewMode === 'split' ? ((100 - splitRatio) + '%') : '100%', display: viewMode === 'edit' ? 'none' : 'flex' }}
          ...
          <div className="flex items-center justify-between px-3 py-1 bg-slate-50 dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex-shrink-0 transition-colors">
            <span className="text-slate-600 dark:text-zinc-300 font-medium text-xs flex items-center gap-1.5"><i className="fa-solid fa-eye"></i> Live Preview</span>
            {viewMode === 'split' && (
          ...
          <div className="flex-1 overflow-y-auto min-h-0 min-w-0 relative bg-white dark:bg-zinc-950 transition-colors">
            <div className="p-4 md:p-8 md:px-12">
              <div ref={previewRef} className="markdown-body max-w-4xl mx-auto" style={{ fontFamily: previewFont }}
